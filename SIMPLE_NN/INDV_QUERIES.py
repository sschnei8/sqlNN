...
SELECT CURRENT_TIMESTAMP AS INSERT_TIME
     ...
     , CASE WHEN OUTPUT_NODE = 1 THEN .01
            WHEN OUTPUT_NODE = 2 THEN .99 END AS ACTUAL_VALUE
FROM L2_ACTIVATION
)
"""
...
SELECT W.HIDDEN_LAYER
     ...
     , W.HIDDEN_VALUE
     , W.WEIGHT
     , W.WEIGHT - (0.5 * G.BACK_PROP_GRADIENT_L2) AS UPDATED_WEIGHT -- LEARNING RATE OF .5
...
WHERE W.HIDDEN_LAYER = 1 -- ONLY THE TWO WEIGHTS OF INTEREST
),
...
SELECT L.INPUT_NODE
     ...
     , L.WEIGHT
     , A.NET_W
     , L.WEIGHT - (.5 * A.NET_W) AS UPDATED_WEIGHT
FROM SIMPLE_NN.L1_WEIGHTS L
         LEFT JOIN ADJUSTMENT A ON 1 = 1
)
